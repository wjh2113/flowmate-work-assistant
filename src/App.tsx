...
export default function App(){
  ...
  const syncVoiceTask=(task:Task)=>{const current=cloudContext.current;if(current.session&&current.teamId)upsertTask(toCloud(task,current.teamId,current.session.user.id)).catch(error=>notify(`语音任务云端同步失败：${error.message}`));else saveFileTask(task).catch(error=>notify(`数据保存失败：${error.message}`))};
...
function AIReport({report,loading,error,aiReady,generate,addTomorrow,openSettings,editPending,onRetryEdit}:{report:DailyReport|null;loading:boolean;error:string;aiReady:boolean|null;generate:()=>void;addTomorrow:()=>void;openSettings:()=>void;editPending?:{id:string;stage:string;error?:string};onRetryEdit:()=>void}){
  ...
  const handleGenerate=()=>{if(aiReady===false){if(!native)openSettings();return}generate()};
  ...
  if(!report)return <section className="ai-report empty-report"><div className="ai-orb">≡</div><div><b>{aiReady===false?(native?'AI 服务未就绪':'复盘服务待配置'):'整理今天的工作'}</b><p>{error||(native?'请联系管理员在后台配置模型与积分。':'汇总完成情况，并安排明天的重要事项。可在「我的 → 设置」配置自动生成时间。')}</p><button type="button" onClick={handleGenerate}>{aiReady===false?(native?'知道了':'查看配置说明'):'开始整理'}</button></div></section>;
...
function AIPeriodReport({kind,report,loading,error,aiReady,generate,addNext,openSettings,rangeLabel,editPending,onRetryEdit}:{kind:'weekly'|'monthly';report:PeriodReport|null;loading:boolean;error:string;aiReady:boolean|null;generate:()=>void;addNext?:()=>void;openSettings:()=>void;rangeLabel:string;editPending?:{id:string;stage:string;error?:string};onRetryEdit:()=>void}){
  ...
  if(!report)return <section className="ai-report empty-report"><div className="ai-orb">≡</div><div><b>{aiReady===false?(native?'AI 服务未就绪':'复盘服务待配置'):`生成${label}复盘`}</b><p>{error||(native?'请联系管理员在后台配置模型与积分。':`${rangeLabel} · AI 将整理${label}进展，并给出${nextLabel}计划。`)}</p><button type="button" onClick={handleGenerate}>{aiReady===false?(native?'知道了':'查看配置说明'):'开始生成'}</button></div></section>;
...
function AppSettings({schedule,onScheduleSaved,onClose,onModelSaved,askConfirm}:{schedule:AutoSchedule;onScheduleSaved:(next:AutoSchedule)=>void;onClose:()=>void;onModelSaved:()=>void;askConfirm:(message:string,options?:{title?:string;confirmLabel?:string;cancelLabel?:string;danger?:boolean})=>Promise<boolean>}){
  ...
  return <div className="model-settings app-settings"><div className="settings-head"><div><small>应用设置</small><h2>设置</h2></div><button type="button" onClick={onClose} aria-label="关闭">×</button></div><div className="settings-tabs" role="tablist"><button type="button" role="tab" aria-selected={tab==='schedule'} className={tab==='schedule'?'active':''} onClick={()=>setTab('schedule')}>自动作业</button>{!native&&<button type="button" role="tab" aria-selected={tab==='model'} className={tab==='model'?'active':''} onClick={()=>setTab('model')}>大模型</button>}</div>{tab==='model'&&!native?<ModelSettings onClose={onClose} onSaved={onModelSaved} askConfirm={askConfirm}/>:<ScheduleSettings schedule={schedule} onSaved={onScheduleSaved} onClose={onClose}/>}</div>;
}
